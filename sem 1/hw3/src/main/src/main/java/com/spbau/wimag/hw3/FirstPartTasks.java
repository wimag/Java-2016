...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.filter(((Predicate<Album>) new HashSet<>()::add).negate()).count();
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparingDouble((a) -> ((Album) a).getTracks().stream()
                .mapToInt(Track::getRating).average().orElse(0)).reversed()).collect(Collectors.toList());
    }
